...
void main() {
  group('ActiveChatNotifier', () {
    ...
    test('isActive returns true for active chat', () {
      container.read(activeChatProvider.notifier).set('group123');
      expect(container.read(activeChatProvider.notifier).isActive('group123'), isTrue);
    });

    test('isActive returns false for different chat', () {
      container.read(activeChatProvider.notifier).set('group123');
      expect(container.read(activeChatProvider.notifier).isActive('other'), isFalse);
    });

    test('isActive returns false when no chat is active', () {
      expect(container.read(activeChatProvider.notifier).isActive('group123'), isFalse);
    });
  ...
  group('ActiveChatObserver', () {
    late ProviderContainer container;
    late ActiveChatObserver observer;

    setUp(() {
      container = ProviderContainer();
      observer = ActiveChatObserver(container.read(activeChatProvider.notifier));
    });
    ...
    group('didReplace', () {
      ...
      test('clears when newRoute is null', () {
        container.read(activeChatProvider.notifier).set('group123');
        observer.didReplace(newRoute: null, oldRoute: fakeRoute('/chats/group123'));
        expect(container.read(activeChatProvider), isNull);
      });
    });
